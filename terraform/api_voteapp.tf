resource "opentelekomcloud_apigw_api_v2" "api_voteapp" {
  ...
  # TRIGGER-Name
  name = replace(format("%s_%s_apig_trigger_voteapp", var.prefix, var.appname), "-", "_")
  ...
  description      = format("Created by script for %s-%s-%s", var.prefix, var.appname, "voteapp")
...
##########################################################
resource "opentelekomcloud_apigw_api_publishment_v2" "voteapp_default" {
  gateway_id     = local.API_GATEWAY_INSTANCE_ID
  environment_id = local.ENV_ID
  api_id         = opentelekomcloud_apigw_api_v2.api_voteapp.id
  version_id     = opentelekomcloud_apigw_api_v2.api_voteapp.version
}
